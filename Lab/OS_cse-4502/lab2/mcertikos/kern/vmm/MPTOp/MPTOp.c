...
{
    unsigned int pde_index = PDE_ADDR(vaddr); // creates the page directory entry index

    if (get_pdir_entry(proc_index, pde_index) != 0)
    {
        return get_ptbl_entry(proc_index, pde_index, PTE_ADDR(vaddr)); // returns the page table entry
    }
    else
    {
        return 0;
